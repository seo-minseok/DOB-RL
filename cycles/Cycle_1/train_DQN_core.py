"""
train_DQN_core.py  (Cycle 1 local copy)

base/original/train_DQN_core.py 와 동일하나 td_loss 수집 추가.
변경점:
  - episode 단위 평균 td_loss 수집 → episode_td_loss_vector
...
RESULTS_DIR = Path(os.getenv("DQN_RESULTS_DIR", str(PROJECT_ROOT / "checkpoints" / "dqn")))
...
def train_DQN_core(run_idx: int, num_episodes: int):
    ...
    episode_td_loss_vector           : list[float]  ← 추가: episode 평균 td_loss
    """
