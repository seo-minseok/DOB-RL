"""
Multi_Seed_MBRL_Exp.py

PyTorch + Gymnasium implementation of Multi_Seed_MBRL_Exp.m
Runs train_MBRL_core across multiple random seeds in parallel,
...
def _run_single(args):
    ...
    print(f"[Run {run_idx}] MBRL training started...")
    rewards, steps = train_MBRL_core(run_idx, num_episodes)
...
if __name__ == "__main__":
    ...
    print(f"=== Vanilla MBRL Multi-Seed Training Started (Runs: {num_runs}) ===")
    ...
    result_pkl_path = RESULTS_DIR / "MBRL_MultiSeed_Result.pkl"
    ...
    ax.plot(common_steps, mean_curve, "b-", linewidth=2, label="Mean Reward")
    ax.axhline(y=target_score, color="black", linestyle="--", linewidth=1, label="Target")

    ax.set_xlabel("Total Environmental Steps")
    ax.set_ylabel("Episode Reward")
    ax.set_title(f"Vanilla MBRL Performance (Avg over {num_runs} Seeds)")
    ax.legend(loc="lower right")
    ax.set_ylim(-50, 550)

    plt.tight_layout()
    plot_path = RESULTS_DIR / "MBRL_MultiSeed_Result.png"
