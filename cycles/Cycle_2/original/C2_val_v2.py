"""
Python port of C2_val_v2.m.

The rollout logic, uncertainty calculation, hero-conditioned heatmap, and
video rendering order are kept aligned with the original MATLAB script.
"""
...
from train_DOB_core import (
    ACT_ELEMENTS,
    NormalizedRBFModel,
    QNetwork,
    ResidualDxNet,
    default_cartpole_params,
    predict_next_obs_dob,
)


FOLDER = Path("Node32")
FILE_NAME = "DOB_MBRL_Checkpoint_Ep20.pt"
...
def resolve_checkpoint_path(folder: Path, file_name: str) -> Path:
    direct_path = folder / file_name
    if direct_path.exists():
        return direct_path

    if file_name.endswith(".mat"):
        candidate = folder / (Path(file_name).stem + ".pt")
        if candidate.exists():
            return candidate

    raise FileNotFoundError(
        f"Checkpoint not found: '{direct_path}'. "
        "Python port expects a PyTorch checkpoint (.pt)."
    )
...
def load_models(checkpoint_path: Path):
    ...
    p_nom = checkpoint.get("p_nom", default_cartpole_params())
...
def main():
    checkpoint_path = resolve_checkpoint_path(FOLDER, FILE_NAME)
    policy, res_net, uncert_model, p_nom = load_models(checkpoint_path)
    print("Models loaded for Video Generation.")
    ...
    obs0_batch = obs0_batch + rng.standard_normal((4, NUM_ROLLOUTS)).astype(np.float32) * noise_scale
    ...
    plt.close(fig)
    print(f"Video successfully saved as {video_path}")
