...
PyTorch + Gymnasium implementation of train_MBRL_core.m
...
RESULTS_DIR = Path(os.getenv("MBRL_RESULTS_DIR", str(PROJECT_ROOT / "Python" / "MBRL")))
...
# ============================================================
# Transition Network (World Model)
# MATLAB: state_path + action_path → concat → FC(32)→ReLU→FC(32)→ReLU→FC(4)
#         hidden1=32, hidden2=32
# ============================================================
class TransitionNetwork(nn.Module):
    def __init__(self, num_observations: int = 4, num_act_features: int = 1):
        ...
        # MATLAB: concatenationLayer(1,2) → fullyConnectedLayer(hidden1) → ...
    ...
    def forward(self, obs: torch.Tensor, act: torch.Tensor) -> torch.Tensor:
        # obs: (batch, 4),  act: (batch, 1)
        obs_norm = 2.0 * (obs - self.obs_min) / (self.obs_max - self.obs_min) - 1.0
        act_norm = 2.0 * (act - self.act_min) / (self.act_max - self.act_min) - 1.0
        x = torch.cat([obs_norm, act_norm], dim=-1)
        x = torch.relu(self.fc1(x))
        x = torch.relu(self.fc2(x))
        return self.fc3(x)   # dx (predicted delta observation)


# ============================================================
# Replay Buffer  (MATLAB: myBuffer / myModelBuffer structs)
# ============================================================
class ReplayBuffer:
    ...
    # --- single-sample store (used in real-env loop) ---
    ...
    # --- batch store with wrap-around (used for model buffer) ---
...
# MATLAB: trainTransitionModel  (sgdmupdate, numEpochs=5)
# ============================================================
def train_transition_model(model: TransitionNetwork,
    ...
    for _ in range(num_epochs):
        ...
        for it in range(num_iterations):
            ...
            dx_true = next_t - obs_t           # MATLAB: TargetDx = nextObs - obs
...
# ============================================================
# Generate Samples (Model Rollouts)
# MATLAB: generateSamples(...)
#   - Horizon scheduling: linear from minHorizon=1 to maxHorizon=5
#     over episodes [20, 200]
#   - Ensemble of 3 models, shuffled order each horizon step
#   - Epsilon-greedy with epsilon_min_model=0.1 floor
# ============================================================
def generate_samples(real_buffer:   ReplayBuffer,
                     model_buffer:  ReplayBuffer,
                     transition_models,
                     q_network:     QNetwork,
                     epsilon:       float,
                     options:       dict,
                     episode_ct:    int) -> ReplayBuffer:

    num_models         = len(transition_models)
    min_horizon        = 1
    max_horizon        = options['horizon_length']       # 5
    start_ep           = 20
    end_ep             = 200
    num_iterations     = options['num_generate_sample_iteration']  # 20
    mini_batch_size    = options['mini_batch_size']
    epsilon_min_model  = options['epsilon_min_model']

    # --- linear horizon schedule ---
    ...
    act_elements_t = torch.tensor(ACT_ELEMENTS)   # [-10., 10.]

    for _ in range(num_iterations):
        predicted_next_obs = [None] * num_models   # per iteration-index chain

        for h in range(horizon_length):
            # MATLAB: sampledModelIndices = randperm(numModels)  (re-shuffled each horizon step)
            ...
            for mi, model_id in enumerate(model_order):
                ...
                sampled_force = act_elements_t[action_indices].unsqueeze(1)  # (batch, 1)

                # Random actions for epsilon replacement
                # MATLAB: actInfo.usample(miniBatchSize)  →  uniform random from Elements
                rand_idx    = np.random.randint(0, 2, mini_batch_size)
                random_force = act_elements_t[rand_idx].unsqueeze(1)

                # MATLAB: replaceActionIndices = rand(1,B) < max(epsilon, epsilonMinModel)
                ...
                # World-model rollout: predict delta, then next obs
                with torch.no_grad():
...
# MATLAB: sampleMixedMinibatch(...)
#   realRatio=0.2  →  20 % real, 80 % model
# ============================================================
def sample_mixed_minibatch(model_trained:    bool,
                           real_ratio:       float,
                           mini_batch_size:  int,
                           real_buffer:      ReplayBuffer,
                           model_buffer:     ReplayBuffer):
...
# MATLAB: train_MBRL_core(runIdx, numEpisodes)
# ============================================================
def train_MBRL_core(run_idx: int, num_episodes: int):
    ...
    # --------------------------------------------------
    # 3. Transition Models — ensemble of 3, hidden=32/32
    # MATLAB: sgdmupdate(lr=1e-2, momentum=0.9), separate velocity per model
    # --------------------------------------------------
    num_models = 3
    transition_models = [
    ...
    act_elements_t = torch.tensor(ACT_ELEMENTS)   # used for action mask
    ...
    for episode_ct in range(1, num_episodes + 1):
        ...
        # --- Train world models & fill model buffer ---
        ...
        for step_ct in range(1, max_steps_per_ep + 1):
            ...
            is_done = bool(terminated)   # pole failure — not truncation

            # Custom shaped reward
            ...
            # --- Q-network update ---
            # MATLAB: if mod(stepCt, updateInterval)==0
            #   totalUpdatesToRun = numGradientSteps * updateInterval
            if (step_ct % update_interval == 0) and (total_step_ct > warm_start_samples):
                ...
                for _ in range(total_updates):
                    ...
                    # MATLAB: tau*w + (1-tau)*t  where w=critic, t=target
                    with torch.no_grad():
