"""
run_multi_seed_mbrl.py — Vanilla MBRL 멀티 시드 병렬 학습 진입점
MATLAB: Multi_Seed_MBRL_Exp.m
Usage:
  python run_multi_seed_mbrl.py [--num-seeds 16]
"""
import argparse
import csv
...
# Cycle 1 로컬 train_MBRL_core 사용 (td_loss 반환 버전)
...
def _run_single(args):
    run_idx, num_episodes, mbrl_ckpt_dir = args

    # train_MBRL_core의 RESULTS_DIR은 모듈 로드 시점에 결정되므로
    # 환경변수를 먼저 설정한 뒤 임포트한다 (자식 프로세스에서 fresh import)
    os.environ['MBRL_RESULTS_DIR'] = mbrl_ckpt_dir
    from train_MBRL_core import train_MBRL_core

    print(f'[Run {run_idx:02d}] Vanilla MBRL training started...')
    rewards, steps, td_losses = train_MBRL_core(run_idx, num_episodes)
    print(f'[Run {run_idx:02d}] Done.')
    return rewards, steps, td_losses


def parse_args():
    parser = argparse.ArgumentParser(description='Vanilla MBRL multi-seed training')
...
def main():
    ...
    mbrl_ckpt_dir    = os.path.join(script_dir, 'checkpoints', 'mbrl')
    os.makedirs(results_dir,   exist_ok=True)
    os.makedirs(mbrl_ckpt_dir, exist_ok=True)
    ...
    print('=' * 60)
    print(f'Vanilla MBRL Multi-Seed Training  (runs={num_runs}, ep={max_episodes})')
    print('=' * 60)
    start_time = time.time()

    num_workers = min(num_runs, multiprocessing.cpu_count())
    args_list   = [(i + 1, max_episodes, mbrl_ckpt_dir) for i in range(num_runs)]
    ...
    # --- pkl 저장 ---
    result_path = os.path.join(results_dir, 'MBRL_MultiSeed_Result.pkl')
    ...
    # --- csv 저장 ---
    csv_path = os.path.join(results_dir, 'MBRL_MultiSeed_Result.csv')
    ...
    # --- figure 자동 생성 ---
    figures_dir = os.path.join(script_dir, 'figures', 'mbrl')
    plot_script = os.path.join(script_dir, 'scripts', 'plot_metrics.py')
